<template>
    <div class="ellipsis-plus" ref="container">
        ...
        ><span
            class="ellipsis-plus-ellipsis"
            ref="ellipsis"
            :style="{ display: show ? 'none' : 'inline-block' }"
            >{{ ellipsis }}</span
        ><button
            class="ellipsis-plus-button"
            v-if="showButton"
            :style="{
                'margin-left': show ? marginLeft + 'px' : collapseMarginLeft + 'px'
            }"
            ref="more"
            @click="handleClick"
        >
            {{ show ? collapseText : expandText }}
        </button>
    </div>
</template>
<script>
export default {
    ...
    mounted() {
        ...
        const everywidth = this.$refs.txt.offsetWidth
        ...
        if (this.showButton) {
            btnWidthExpand = btnWidth = Math.ceil(
                parseFloat(
                    getComputedStyle(this.$refs.more, null)['width'].replace('px', '')
                )
            )
            this.$refs.ellipsis.style.display = 'none'
            let left = 0
            const btnClone = this.$refs.more.cloneNode()
            if (this.expandText !== this.collapseText) {
                ...
                btnWidthExpand = Math.ceil(
                    parseFloat(
                        getComputedStyle(btnClone, null)['width'].replace('px', '')
                    )
                )
        ...
        const style = getComputedStyle(this.$refs.container, null)
        ...
        const initNum = Math.floor(
            (containerWidth * this.line - btnWidth - ellipsisWidth) / everywidth
        )
        let increase = 1
        ...
        for (let i = initNum; i < this.text.length; i = i + increase) {
            ...
            if (
                increase === 1 &&
                Math.round(this.$refs.container.offsetHeight / lineHeight) > this.line
            ) {
                this.tmpTxt = this.text.substr(0, i - 1)
                this.$refs.txt.innerHTML = this.tmpTxt
                if (this.showButton) {
                    const left = this.$refs.more.offsetLeft
